...
const ImportParentsModal = ({ onClose, onSuccess }) => {
  ...
  useEffect(() => {
    const fetchInfo = async () => {
      try {
        const res = await fetch(buildApiUrl('/users/import-parents/info/'), {
  ...
  const handleDownloadTemplate = () => {
    const headers = ['first_name', 'last_name', 'email', 'phone_number', 'gender'];
    if (usernameMode === 'xlsx') headers.push('username');
    headers.push('child_usernames', 'middle_name', 'date_of_birth');

    const exampleRow = [
      'John', 'Doe', 'john.doe@mail.com', '08012345678', 'Male',
    ];
    if (usernameMode === 'xlsx') exampleRow.push('john.doe');
    exampleRow.push('STU001,STU002', '', '');
  ...
  const handleValidate = async () => {
    ...
    try {
      const res = await fetch(buildApiUrl('/users/import-parents/validate/'), {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });
  ...
  const handleConfirmImport = async () => {
    ...
    try {
      const res = await fetch(buildApiUrl('/users/import-parents/confirm/'), {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });
      ...
      <div className="import-modal-overlay" onClick={onClose}>
        <div className="import-modal import-modal-small" onClick={e => e.stopPropagation()}>
          <div className="import-modal-header">
            <h2>Import Parents</h2>
            <button className="import-modal-close" onClick={onClose}><X size={20} /></button>
          </div>
          <div className="import-upgrade-prompt">
            <AlertCircle size={48} />
            <h3>Feature Not Available</h3>
            <p>XLSX parent import is available on Standard and Premium plans. Upgrade your subscription to access this feature.</p>
        ...
        <div className="import-modal-body">
          ...
          {step === 1 && (
            <div className="import-step-content">
              ...
              <p className="import-step-desc">Choose how usernames will be assigned for the imported parents.</p>
              {maxImportRows > 0 && (
                <div className="import-limit-notice">
                  <Info size={16} />
                  <span>Your plan allows up to <strong>{maxImportRows} parents</strong> per import.</span>
                </div>
              )}
              ...
              <div className="import-instructions">
                <div className="import-instruction-section">
                  ...
                  <div className="import-columns-list">
                    ...
                    <span className="import-col required">phone_number</span>
                ...
                <div className="import-instruction-section">
                  ...
                  <div className="import-columns-list">
                    <span className="import-col optional">child_usernames</span>
                ...
                <div className="import-instruction-section">
                  <h4><AlertCircle size={16} /> Linking Children</h4>
                  <p>
                    In the <code>child_usernames</code> column, enter each child's school username separated by commas.
                    Usernames must match existing students in this school.
                  </p>
                  <p><strong>Example:</strong> <code>STU001,STU002,STU003</code></p>
                  <p>Leave the column empty if the parent has no children to link yet.</p>
                </div>
                ...
                <div className="import-instruction-section">
                  ...
                  <p>If included, use: <code>YYYY-MM-DD</code> (e.g. 1980-06-15)</p>
                </div>
              </div>
            ...
            <div className="import-step-content">
              ...
              <div className="import-summary">
                <div className="import-summary-item success">
                  <CheckCircle size={20} />
                  <span><strong>{validationResults.valid_count}</strong> parents ready to import</span>
                </div>
            ...
            <div className="import-step-content">
              ...
              {importResults.created_count > 0 && (
                <p className="import-results-note">
                  Verification emails have been sent to all created parents. They will need to verify their email and set up a password before logging in.
                </p>
              )}
            ...
            <>
              ...
              >
                {importing ? (
                  ...
                  <>Confirm & Import {validationResults?.valid_count} Parents</>
                )}
              </button>
            </>
          )}
...
export default ImportParentsModal;
