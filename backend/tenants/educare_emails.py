"""
EduCare platform email service.

Sends branded emails from the EduCare platform (NOT from individual schools).
...
EDUCARE_SENDER_NAME = 'EduCare'
EDUCARE_SENDER_EMAIL = 'office@figilschools.com'
EDUCARE_ACCENT_COLOR = '#3b82f6'
EDUCARE_FRONTEND_URL = getattr(settings, 'FRONTEND_URL', 'https://educare.com')


def _build_educare_html(heading, body_html, cta_text=None, cta_url=None):
    """Build a professional EduCare-branded HTML email."""
    cta_block = ''
    if cta_text and cta_url:
        cta_block = f'''
                    <div style="text-align: center; margin-top: 30px;">
                        <a href="{cta_url}" style="display: inline-block; padding: 14px 32px;
                           background-color: {EDUCARE_ACCENT_COLOR}; color: #ffffff;
...
<body style="margin: 0; padding: 0; font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Roboto, sans-serif; background-color: #f0f4f8; color: #1a202c;">
    <div style="max-width: 600px; margin: 0 auto; padding: 40px 20px;">
        <!-- Header -->
        <div style="background: linear-gradient(135deg, {EDUCARE_ACCENT_COLOR}, #1d4ed8); padding: 32px; text-align: center; border-radius: 12px 12px 0 0;">
            <!-- EduCare logo (inline SVG — no external image dependency, no whitespace) -->
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 300 80" width="210" height="56" style="display:inline-block; margin-bottom: 8px;" role="img" aria-label="EduCare">
              <defs>
        ...
        <div style="text-align: center; padding: 24px 0; color: #a0aec0; font-size: 12px;">
            <p style="margin: 0 0 4px;">This is an official email from EduCare.</p>
            <p style="margin: 0 0 4px;">Please do not reply to this email.</p>
            <p style="margin: 0;">&copy; EduCare School Management Platform</p>
        </div>
    </div>
</body>
</html>'''
...
def send_educare_email(recipient_email, recipient_name, subject, heading, body_html, cta_text=None, cta_url=None):
    """
    Send a branded EduCare platform email.

    This bypasses school branding and email quotas.
    """
    try:
        html_content = _build_educare_html(heading, body_html, cta_text, cta_url)
        from_email = f"{EDUCARE_SENDER_NAME} <{EDUCARE_SENDER_EMAIL}>"
        email = EmailMessage(
            subject=f"[EduCare] {subject}",
            body=html_content,
        ...
        email.send()
        logger.info(f"EduCare email sent to {recipient_email}: {subject}")
        return True
    except Exception as e:
        logger.error(f"Failed to send EduCare email to {recipient_email}: {e}")
        return False
...
def _get_dashboard_url(subscription):
    ...
    return f"{EDUCARE_FRONTEND_URL}/{subscription.school.slug}/admin/dashboard"


def _get_pricing_url():
    """Get the pricing page URL."""
    return f"{EDUCARE_FRONTEND_URL}/pricing"
...
def send_onboarding_welcome_email(subscription, registration_type='trial'):
    ...
    portal_url = f"{EDUCARE_FRONTEND_URL}/portal"
    ...
    if registration_type in ('trial', 'termly_trial'):
        ...
        intro = (
            ...
            f"Welcome to the EduCare family! To make sure your school is set up for success, "
    ...
    body_html = f'''
        ...
        <div style="background:#eff6ff; border-left:4px solid #2563eb; padding:18px 20px;
            ...
            <p style="margin:0; color:#374151; font-size:14px;">
                This is a free, included service for all EduCare schools — no extra charge, no strings attached.
            </p>
        </div>
        ...
        <p style="color:#6b7280; font-size:14px;">
            ...
            you can reach us at <a href="mailto:{EDUCARE_SENDER_EMAIL}" style="color:#2563eb;">{EDUCARE_SENDER_EMAIL}</a>.
        </p>
    '''
    ...
    for recipient in recipients:
        ...
        send_educare_email(
            ...
            subject=f'Welcome to EduCare — Your Onboarding Expert Will Reach Out Within 24 Hours',
            heading=f'Welcome to EduCare, {school_name}! 🎓',
...
def send_expired_lockout_email(subscription):
    ...
    body = f'''
        ...
        Access to your school on EduCare has been <strong>suspended</strong>.</p>
        <div style="background-color: #fee2e2; border-left: 4px solid #dc2626; padding: 12px 16px; margin: 20px 0; border-radius: 4px;">
...
def send_educare_welcome_email(subscription):
    ...
    body = f'''
        <p>Dear <strong>{school_name}</strong> team,</p>
        <p>Welcome to <strong>{plan_name}</strong>! Your subscription is now active. Thank you for choosing EduCare.</p>
...
def send_educare_payment_confirmation(subscription, payment):
    """Send payment confirmation from EduCare branding."""
    school_name = subscription.school.name
    plan_name = subscription.plan.display_name
    ...
    body = f'''
        ...
        <p>Your subscription is now active. Thank you for your continued trust in EduCare.</p>
    '''
...
def send_educare_cancellation_email(subscription):
    ...
    body = f'''
        ...
        <p>Changed your mind? You can resubscribe at any time to continue using EduCare.</p>
    '''
...
def send_educare_payment_failed_email(subscription):
