...
from logs.email_service import send_verification_email, send_password_reset_email
...
def forgot_password(request):
    ...
    try:
        ...
        return Response({
            'status': 'success',
            'message': 'If an account exists with this information, a password reset email has been sent.'
        }, status=status.HTTP_200_OK)
    ...
    if not user.email:
        return Response({
            'status': 'error',
            'message': 'This account does not have an email address. Please contact administration.'
        }, status=status.HTTP_400_BAD_REQUEST)
